...
- `/生图额度`
  - 查看当前用户最近 1 分钟、1 小时、滚动 24 小时的成功生图次数和执行中任务。
...
| `max_requests_per_hour`      | number  | `30`                                          | 单个用户滚动一小时的最大成功生图次数。                            |
| `max_requests_per_day`       | number  | `100`                                         | 单个用户滚动 24 小时的最大成功生图次数，重启不会清零。            |
