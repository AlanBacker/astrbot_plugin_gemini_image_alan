...
- `/生图额度`
  - 群聊中查看当前群额度，私聊中查看当前用户额度；未授权会提示没有权限。
...
- ✅ **会话独立额度**：群聊统一消耗当前群额度，私聊只消耗当前用户额度，两者互不影响。
- ✅ **持久化成功计数**：仅成功发送图片后计数，失败不计数；命令与 LLM 调用共用限额，重启不会清零。
...
| `max_requests_per_day`       | number  | `100`                                         | 当前群或私聊用户滚动 24 小时的最大成功生图次数。                  |
...
| `users`                   | list    | `[]`            | 私聊用户黑/白名单；只用于私聊权限。                                  |
| `groups`                  | list    | `[]`            | 群聊黑/白名单；群内权限和共享额度均以群 ID 为准。                    |
